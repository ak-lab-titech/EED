...
class HumanoidData:
    def __init__(self, _id, _position, _rotation):
        self.id = _id # humanoidデータのインデックス番号
        self.position = _position # humanoidの挿入位置
        self.rotation = _rotation # humanoidの向き
...
class MaximumInformationEpisode(Episode):
    ...
    """
    humanoids: List[HumanoidData] = attr.ib(
        default=None, validator=not_none_validator
    )
    add_description: str = attr.ib(
        default=None, validator=not_none_validator
    )
