...
class ThreadedVectorEnvHumanoid(VectorEnvHumanoid):
    r"""Provides same functionality as `VectorEnvHumanoid`, the only difference is it
    runs in a multi-thread setup inside a single process.

    `VectorEnvHumanoid` runs in a multi-proc setup. This makes it much easier to debug
    when using `VectorEnvHumanoid` because you can actually put break points in the
