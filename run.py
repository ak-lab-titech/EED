...
def main():
    ...
    parser.add_argument(
        "--run-type",
        choices=["train", "train_exp", "eval", "eval_exp", "random", "collect"],
        default=None,
...
def test(run_type: str, area_reward_type: str, opts=None):    
    ...
    if agent_type in ["oracle", "oracle-ego", "no-map"]:
        if run_type in ["train_exp", "eval_exp"]:
            trainer_init = baseline_registry.get_trainer("oracle_exploration")
        else:
    ...
    try:
        if run_type in ["train", "train_exp"]:
            #フォルダがない場合は、作成
        ...
        elif run_type in ["eval", "eval_exp"]:
